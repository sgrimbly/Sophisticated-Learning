...
for (name in names(file_paths)) {
  ...
  # Use only the first 45 rows of data
  df <- df[10:45, ]
...
write.csv(anova_results, file = "anova_10_45.csv", row.names = FALSE)

# Pairwise comparisons with estimated marginal means
em <- emmeans(model, pairwise ~ Model)
write.csv(em$emmeans, file = "em_10_45.csv", row.names = FALSE) # Save only emmeans results
