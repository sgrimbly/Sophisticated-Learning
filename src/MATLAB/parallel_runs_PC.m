...
function configs = read_configurations(filename)
    ...
    while ~feof(fid)
        ...
        if contains(line, 'Grid Size')
            cfgData = textscan(line, 'Grid Size: %d, Horizon: %d, Hill: %d, Start Position: %d, Food(%[^)]), Water(%[^)]), Sleep(%[^)]');
            grid_size = cfgData{1};
            horizon = cfgData{2};
            hill_pos = cfgData{3};
            start_position = cfgData{4};
            food_sources = str2num(cfgData{5}{1});
            water_sources = str2num(cfgData{6}{1});
            sleep_sources = str2num(cfgData{7}{1});
            configs{end+1} = struct('grid_size', grid_size, 'horizon', horizon, 'hill_pos', hill_pos, 'start_position', start_position, ...
