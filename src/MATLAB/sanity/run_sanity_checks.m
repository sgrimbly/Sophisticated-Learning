...
function sanity_preference_scaling()
    ...
    weight = [0.5, 1, 2, 5];
    extrinsic_weight = zeros(size(weight));

    for i = 1:numel(weight)
        C_scaled = C{2} * weight(i);
        extrinsic_weight(i) = o_food * C_scaled(:);
    end

    assert(all(diff(abs(extrinsic_weight)) > 0), 'Preference scaling sanity check failed: |extrinsic| did not increase with weight.');

    inv_precision = [1, 2, 5, 10];
    extrinsic_inv = zeros(size(inv_precision));

    for i = 1:numel(inv_precision)
        C_scaled = C{2} / inv_precision(i);
        extrinsic_inv(i) = o_food * C_scaled(:);
    end

    assert(all(diff(abs(extrinsic_inv)) < 0), 'Preference scaling sanity check failed: |extrinsic| did not decrease with inverse precision.');
...
function sanity_config_hash_changes_with_config()
    ...
    id3 = config_hash(cfg);
    cfg.state_selection = 'map';
    id4 = config_hash(cfg);
    assert(~strcmp(id3, id4), 'config_hash() did not change when state_selection changed.');

	    cfg.state_selection = 'sample';
	    cfg.preference_param = 'inverse_precision';
	    id5 = config_hash(cfg);
	    assert(~strcmp(id3, id5), 'config_hash() did not change when preference_param changed.');

	    cfg.preference_param = 'weight';
	    cfg.real_smoothing = false;
	    id6 = config_hash(cfg);
	    assert(~strcmp(id3, id6), 'config_hash() did not change when real_smoothing changed.');

		    cfg.real_smoothing = true;
		    cfg.adaptive_likelihood_in_plan = true;
		    id7 = config_hash(cfg);
		    assert(~strcmp(id3, id7), 'config_hash() did not change when adaptive_likelihood_in_plan changed.');

		    cfg.adaptive_likelihood_in_plan = false;
		    cfg.learning_prune_threshold = 0;
		    id8 = config_hash(cfg);
		    assert(~strcmp(id3, id8), 'config_hash() did not change when learning_prune_threshold changed.');
