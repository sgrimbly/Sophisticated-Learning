function [G, P, short_term_memory, best_actions, memory_accessed, efe_components] = tree_search_frwd_SL_noSmooth(short_term_memory, O, P, a, A, y, B, b, t, T, N, t_food, t_water, t_sleep, true_t, chosen_action, true_t_food, true_t_water, true_t_sleep, best_actions, learning_weight, novelty_weight, epistemic_weight, preference_inverse_precision, memory_accessed, varargin)
    ...
    if t > true_t
        ...
        novelty = novelty + kldir(normalise(a_temp(:)), normalise(a_prior(:)));
        ...
        for action = actions
            ...
            for state = likely_states(:)'
                ...
                if short_term_memory(t_food_idx, t_water_idx, t_sleep_idx, state) ~= 0
                    ...
                    % the tree
                    if collect_efe_components
                        [expected_free_energy, P, short_term_memory, best_actions, memory_accessed, efe_child] = tree_search_frwd_SL_noSmooth(short_term_memory, O, P, a, A, y, B, b, t + 1, T, N, t_food, t_water, t_sleep, true_t, chosen_action, true_t_food, true_t_water, true_t_sleep, best_actions, learning_weight, novelty_weight, epistemic_weight, preference_inverse_precision, memory_accessed, collect_efe_components, adaptive_likelihood_in_plan, learning_prune_threshold);
