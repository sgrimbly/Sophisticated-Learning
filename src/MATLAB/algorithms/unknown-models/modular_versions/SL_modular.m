function [survived] = SL_modular(seed, grid_size, start_position, hill_pos, food_sources, water_sources, sleep_sources, weights, num_states, num_trials, grid_id, results_file_override)
    ...
    result_file = fullfile(directory_path, sprintf('SL_Seed_%d_GridID_%s_Cfg_%s_%s.txt', seed, grid_id_safe, config_id, current_time));
    ...
    stateFile = fullfile(directory_path, sprintf('SL_Seed_%d_GridID_%s_Cfg_%s.mat', seed, grid_id_safe, config_id));
    ...
    if ~isempty(results_file_override)
        ...
        stateFile = fullfile(results_dir, sprintf('SL_Seed_%d_GridID_%s_Cfg_%s.mat', seed, grid_id_safe, config_id));
    end
