function [survived] = SL_modular(seed, grid_size, start_position, hill_pos, food_sources, water_sources, sleep_sources, weights, num_states, num_trials, grid_id, results_file_override, max_horizon, run_options)
    ...
    if ~isfield(run_options, 'algorithm_label')
        run_options.algorithm_label = 'SL';
    end
    ...
    for trial = trial:num_trials
        ...
        while (t < 100 && time_since_food < 22 && time_since_water < 20 && time_since_sleep < 25)
            ...
            [G, Q, short_term_memory, best_actions, memory_accessed, efe_components] = tree_search_frwd_SL(short_term_memory, O, Q, a, A, y, B, B, t, T, t + horizon, time_since_food, time_since_water, time_since_sleep, true_t, chosen_action, time_since_food, time_since_water, time_since_sleep, best_actions, learning_weight, novelty_weight, epistemic_weight, preference_inverse_precision, memory_accessed, run_options.collect_efe_components, run_options.adaptive_likelihood_in_plan, run_options.learning_prune_threshold);
            if compute_policy_sensitivity && novelty_weight ~= 0
                best_actions_alt = [];
                [~, ~, ~, best_actions_alt] = tree_search_frwd_SL(short_term_memory_base, O, Q_base, a, A, y, B, B, t, T, t + horizon, time_since_food, time_since_water, time_since_sleep, true_t, chosen_action, time_since_food, time_since_water, time_since_sleep, best_actions_alt, learning_weight, 0, epistemic_weight, preference_inverse_precision, memory_accessed_base, false, run_options.adaptive_likelihood_in_plan, run_options.learning_prune_threshold);
