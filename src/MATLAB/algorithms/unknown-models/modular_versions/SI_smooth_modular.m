function [survived] = SI_smooth_modular(seed, grid_size, start_position, hill_pos, food_sources, water_sources, sleep_sources, weights, num_states, num_trials, grid_id, results_file_override)
    ...
    result_file = fullfile(directory_path, sprintf('SI_smooth_Seed_%d_GridID_%s_Cfg_%s_%s.txt', seed, grid_id_safe, config_id, current_time));
    ...
    stateFile = fullfile(directory_path, sprintf('SI_smooth_Seed_%d_GridID_%s_Cfg_%s.mat', seed, grid_id_safe, config_id));
    ...
    if ~isempty(results_file_override)
        ...
        stateFile = fullfile(results_dir, sprintf('SI_smooth_Seed_%d_GridID_%s_Cfg_%s.mat', seed, grid_id_safe, config_id));
    end
    ...
    for trial = trial:num_trials
        ...
        while (t < 100 && time_since_food < 22 && time_since_water < 20 && time_since_sleep < 25)
            ...
            best_actions = [];
            [G, Q, short_term_memory, best_actions, memory_accessed] = tree_search_frwd_SI_smooth(short_term_memory, O, Q, a, A, y, B, B, t, T, t + horizon, time_since_food, time_since_water, time_since_sleep, true_t, chosen_action, time_since_food, time_since_water, time_since_sleep, best_actions, learning_weight, novelty_weight, epistemic_weight, preference_inverse_precision, memory_accessed);
