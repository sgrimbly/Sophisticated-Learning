function [survived] = SI_smooth_modular(seed, grid_size, start_position, hill_pos, food_sources, water_sources, sleep_sources, weights, num_states, num_trials, grid_id, results_file_override, max_horizon, run_options)
    ...
    if nargin < 8, weights = [0, 40, 1, 10]; end
    ...
    if nargin < 13, max_horizon = 9; end
    if nargin < 14, run_options = struct(); end

    if ~isstruct(run_options)
        error('run_options must be a struct.');
    end
    if ~isfield(run_options, 'algorithm_label')
        run_options.algorithm_label = 'SI_smooth';
    end
    ...
    if ~isfield(run_options, 'collect_efe_components')
        run_options.collect_efe_components = false;
    end
    ...
    algorithm_label = run_options.algorithm_label;
    ...
    if ~isempty(results_file_override)
        ...
        stateFile = fullfile(results_dir, sprintf('%s_Seed_%d_GridID_%s_Cfg_%s.mat', algorithm_label_safe, seed, grid_id_safe, config_id));
    end
    ...
    for trial = trial:num_trials
        ...
        while (t < 100 && time_since_food < 22 && time_since_water < 20 && time_since_sleep < 25)
            ...
            [G, Q, short_term_memory, best_actions, memory_accessed, efe_components] = tree_search_frwd_SI_smooth(short_term_memory, O, Q, a, A, y, B, B, t, T, t + horizon, time_since_food, time_since_water, time_since_sleep, true_t, chosen_action, time_since_food, time_since_water, time_since_sleep, best_actions, learning_weight, novelty_weight, epistemic_weight, preference_inverse_precision, memory_accessed, run_options.collect_efe_components);
            if compute_policy_sensitivity && novelty_weight ~= 0
                best_actions_alt = [];
                [~, ~, ~, best_actions_alt] = tree_search_frwd_SI_smooth(short_term_memory_base, O, Q_base, a, A, y, B, B, t, T, t + horizon, time_since_food, time_since_water, time_since_sleep, true_t, chosen_action, time_since_food, time_since_water, time_since_sleep, best_actions_alt, learning_weight, 0, epistemic_weight, preference_inverse_precision, memory_accessed_base, false);
        ...
        if ~isempty(run_options.progress_queue)
            try
                ...
                send(run_options.progress_queue, struct(...
                    ...
                    'efe_novelty_term_sum', efe_novelty_term_sum, ...
