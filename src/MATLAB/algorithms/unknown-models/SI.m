function [survived] = SI(seed, state_selection)
    ...
    directory_path = fullfile(results_root, 'unknown_model', 'MATLAB', '200trials_data_threefry');
    if ~exist(directory_path, 'dir')
        mkdir(directory_path);
    end

    file_name = fullfile(directory_path, sprintf('SI_Seed_%s_%s.txt', seed_str, current_time));
